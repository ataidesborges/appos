...
class Login extends CI_Controller
{
    ...
    public function verificarLogin()
    {
        ...
        if ($this->form_validation->run() == false) {
            ...
            $this->load->model('appos_model');
            $user = $this->appos_model->check_credentials($email);
            ...
            if ($user) {
                ...
                if (password_verify($password, $user->senha)) {
                    ...
                    $json = ['result' => false, 'message' => 'Os dados de acesso estão incorretos.', 'appos_TOKEN' => $this->security->get_csrf_hash()];
                    echo json_encode($json);
                }
            } else {
                $json = ['result' => false, 'message' => 'Usuário não encontrado, verifique se suas credenciais estão corretass.', 'appos_TOKEN' => $this->security->get_csrf_hash()];
                echo json_encode($json);
            }
        }
        exit();
