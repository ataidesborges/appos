...
class appos extends MY_Controller {
    ...
    public function index()
    {
        ...
        $this->data['ordens_status'] = $this->appos_model->getOsStatus($status);
        $vstatus = array('Aberto', 'Em Andamento', 'Aguardando Peças', 'Aprovado', 'Orçamento');
        $this->data['vendasstatus'] = $this->appos_model->getVendasStatus($vstatus);
        $this->data['lancamentos'] = $this->appos_model->getLancamentos();
        $this->data['ordens_orcamentos'] = $this->appos_model->getOsOrcamentos();
        $this->data['ordens_abertas'] = $this->appos_model->getOsAbertas();
        $this->data['ordens_aprovadas'] = $this->appos_model->getOsAprovadas();
        $this->data['ordens_finalizadas'] = $this->appos_model->getOsFinalizadas();
        $this->data['ordens_aguardando'] = $this->appos_model->getOsAguardandoPecas();
        $this->data['ordens_andamento'] = $this->appos_model->getOsAndamento();
        $this->data['produtos'] = $this->appos_model->getProdutosMinimo();
        $this->data['os'] = $this->appos_model->getOsEstatisticas();
        $this->data['estatisticas_financeiro'] = $this->appos_model->getEstatisticasFinanceiro();
        $this->data['financeiro_mes_dia'] = $this->appos_model->getEstatisticasFinanceiroDia($this->input->get('year'));
        $this->data['financeiro_mes'] = $this->appos_model->getEstatisticasFinanceiroMes($this->input->get('year'));
        $this->data['financeiro_mesinadipl'] = $this->appos_model->getEstatisticasFinanceiroMesInadimplencia($this->input->get('year'));
        $this->data['menuPainel'] = 'Painel';
        $this->data['view'] = 'appos/painel';
    ...
    {
        $this->data['usuario'] = $this->appos_model->getById($this->session->userdata('id_admin'));
        $this->data['view'] = 'appos/minhaConta';
    ...
    {
        $current_user = $this->appos_model->getById($this->session->userdata('id_admin'));

        if (!$current_user) {
            $this->session->set_flashdata('error', 'Ocorreu um erro ao pesquisar usuário!');
            redirect(site_url('appos/minhaConta'));
        }
        ...
        $result = $this->appos_model->alterarSenha($senha);
    ...
    public function pesquisar()
    {
        ...
        $data['results'] = $this->appos_model->pesquisar($termo);
        $this->data['produtos'] = $data['results']['produtos'];
        $this->data['servicos'] = $data['results']['servicos'];
        $this->data['os'] = $data['results']['os'];
        $this->data['clientes'] = $data['results']['clientes'];
        $this->data['view'] = 'appos/pesquisa';
    ...
    public function emitente()
    {
        ...
        $this->data['dados'] = $this->appos_model->getEmitente();
        $this->data['view'] = 'appos/emitente';
    ...
    public function cadastrarEmitente()
    {
        ...
        if ($this->form_validation->run() == false) {
            ...
            redirect(site_url('appos/emitente'));
        } else {
            ...
            $retorno = $this->appos_model->addEmitente($nome, $cnpj, $ie, $cep, $logradouro, $numero, $bairro, $cidade, $uf, $telefone, $email, $logo);
            if ($retorno) {
    ...
    public function editarEmitente()
    {
        ...
        if ($this->form_validation->run() == false) {
            ...
            $retorno = $this->appos_model->editEmitente($id, $nome, $cnpj, $ie, $cep, $logradouro, $numero, $bairro, $cidade, $uf, $telefone, $email);
            if ($retorno) {
    ...
    public function editarLogo()
    {
        ...
        $retorno = $this->appos_model->editLogo($id, $logo);
        if ($retorno) {
            $this->session->set_flashdata('success', 'As informações foram alteradas com sucesso.');
    ...
    public function uploadUserImage()
    {
        ...
        $usuario = $this->appos_model->getById($id);
        ...
        $imageUserPath = $image;
        $retorno = $this->appos_model->editImageUser($id, $imageUserPath);
    ...
    public function emails()
    {
        ...
        $this->data['configuration']['base_url'] = site_url('appos/emails/');
    ...
    public function excluirEmail()
    {
        ...
        if ($id == null) {
            ...
            redirect(site_url('appos/emails/'));
        }
    ...
    public function atualizarBanco()
    {
        ...
        return redirect(site_url('appos/configurar'));
    }

    public function atualizarappos()
    {
        if (!$this->permission->checkPermission($this->session->userdata('permissao'), 'cSistema')) {
            $this->session->set_flashdata('error', 'Você não tem permissão para configurar o sistema');
            redirect(base_url());
        ...
        if (!$this->github_updater->has_update()) {
            $this->session->set_flashdata('success', 'Seu appos já está atualizado!');
        ...
        if ($success) {
            $this->session->set_flashdata('success', 'appos atualizado com sucesso!');
        } else {
            $this->session->set_flashdata('error', 'Erro ao atualizar appos!');
        }
    ...
    public function calendario()
    {
        ...
        $allOs = $this->appos_model->calendario(
            $start,
            $end,
            $status
        );
