...
class Mine extends CI_Controller
{
        ...
        if ($this->form_validation->run() == false) {
            ...
            if ($cliente) {
                ...
                if (password_verify($password, $cliente->senha)) {
                    ...
                    echo json_encode(['result' => false, 'message' => 'Os dados de acesso estão incorretos.', 'appos_TOKEN' => $this->security->get_csrf_hash()]);
                }
            } else {
                echo json_encode(['result' => false, 'message' => 'Usuário não encontrado, verifique se suas credenciais estão corretass.', 'appos_TOKEN' => $this->security->get_csrf_hash()]);
            }
        }
    }
    ...
    public function visualizarOs($id = null)
    {
        ...
        $data['emitente'] = $this->appos_model->getEmitente();
    ...
    private function enviarRecuperarSenha($idClientes, $clienteEmail, $assunto, $token)
    {
        ...
        $dados['emitente'] = $this->appos_model->getEmitente();
